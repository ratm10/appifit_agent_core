...
2. **deviceId 출처** — Android 네이티브 시리얼 핸들러가 있는가?
   ...
   - **Windows 는 항상 설치 UUID 다. OS 값을 식별자로 쓰지 말 것** —
     MachineGuid(`SQMClient\MachineId`)든 다른 레지스트리 값이든, 설치 이미지에
     박혀 있는 값은 sysprep 없이 복제 배포된 PC 들에서 전부 같다. 실제로 서로
     다른 두 매장이 한 기기 행을 번갈아 덮어쓴 사고가 있었다(사유는
     CHANGELOG v1.6.0). "보통은 유일하다" 는 식별자의 조건이 아니다.
